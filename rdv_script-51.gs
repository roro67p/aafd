...
function pageEditCoiff(e) {
  ...
  // Construire l URL de sauvegarde avec tous les params fixes
  var saveBase = url + "?page=savecoiff&pwd=" + e.parameter.pwd + "&nom=" + encodeURIComponent(nom);
  ...
  var h = '<!DOCTYPE html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><meta charset="UTF-8">'
    ...
    + '.sbtn{width:100%;padding:15px;background:#6366F1;color:#fff;border:none;border-radius:10px;font-size:16px;font-weight:700;cursor:pointer;margin-top:6px;font-family:sans-serif;text-decoration:none;display:block;text-align:center}'
    ...
    + '<div class="r2">'
    + '<div><span class="lbl">Debut</span><input type="time" id="debut" name="debut" value="' + d.debut + '"></div>'
    + '<div><span class="lbl">Fin</span><input type="time" id="fin" name="fin" value="' + d.fin + '"></div>'
    + '</div></div>'
    ...
    + '<input type="text" id="conges" name="conges" placeholder="2026-04-15, 2026-04-20">'
    + '<div class="r2" style="margin-top:12px">'
    + '<div><span class="lbl">Vacances du</span><input type="date" id="vdebut" name="vdebut"></div>'
    + '<div><span class="lbl">Au</span><input type="date" id="vfin" name="vfin"></div>'
    + '</div></div>'
    + '<div class="sec"><div class="st">Prime EU</div>'
    + '<input type="number" id="prime" name="prime" value="' + d.prime + '" placeholder="0">'
    + '</div>'
    + '</form>'
    + '<a href="#" onclick="save();return false;" class="sbtn">Enregistrer</a>'
    ...
    + '  for (var i=0; i<cbs.length; i++) { if(cbs[i].checked) joffs.push(cbs[i].value); }'
    + '  var u = "' + saveBase + '";'
    + '  var u = "' + saveBase + '";'
    + '  u += "&debut=" + encodeURIComponent(document.getElementById("debut").value);'
    + '  u += "&fin=" + encodeURIComponent(document.getElementById("fin").value);'
    + '  u += "&prime=" + encodeURIComponent(document.getElementById("prime").value);'
    + '  u += "&joff=" + encodeURIComponent(joffs.join(","));'
    + '  u += "&conges=" + encodeURIComponent(document.getElementById("conges").value);'
    + '  u += "&vdebut=" + encodeURIComponent(document.getElementById("vdebut").value);'
    + '  u += "&vfin=" + encodeURIComponent(document.getElementById("vfin").value);'
