...
function pageEditCoiff(e) {
  ...
  jours.forEach(function(j) {
    joursHTML += '<label style="display:flex;align-items:center;gap:10px;padding:10px;background:#111;border-radius:8px;margin-bottom:6px;cursor:pointer;font-size:14px;font-weight:600">'
      + '<input type="checkbox" name="jour_' + j + '" value="' + j + '" style="width:20px;height:20px;cursor:pointer"> ' + j
      + '</label>';
  });
  ...
  var h = '<!DOCTYPE html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><meta charset="UTF-8">'
    ...
    + '<div class="ct">'
    + '<form action="' + url + '?page=savecoiff&pwd=' + e.parameter.pwd + '&nom=' + encodeURIComponent(nom) + '" method="get">'
    ...
    + '<span class="lbl">Jours specifiques ex: 2026-04-15</span>'
