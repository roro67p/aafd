...
function pageEditCoiff(e) {
  ...
  var h = '<!DOCTYPE html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><meta charset="UTF-8">'
    ...
    + '<div class="r2">'
    + '<div><span class="lbl">Debut</span><input type="time" id="debut" value="' + d.debut + '"></div>'
    + '<div><span class="lbl">Fin</span><input type="time" id="fin" value="' + d.fin + '"></div>'
    + '</div></div>'
    ...
    + '<div><span class="lbl">Vacances du</span><input type="date" id="vdebut"></div>'
    + '<div><span class="lbl">Au</span><input type="date" id="vfin"></div>'
    + '</div></div>'
    + '<div class="sec"><div class="st">Prime EU</div>'
    + '<input type="number" id="prime" value="' + d.prime + '" placeholder="0">'
    + '</div>'
    + '<button onclick="save()" class="sbtn">Enregistrer</button>'
    + '<a href="' + adminUrl + '" class="rbtn">Retour sans sauvegarder</a>'
    + '<div class="safe"></div>'
    + '</div>'
    + '<script>'
    + 'function save() {'
    ...
    + '    joff: joffs.join(","),'
    + '    conges: document.getElementById("conges").value,'
    + '    vdebut: document.getElementById("vdebut").value,'
    + '    vfin: document.getElementById("vfin").value'
    + '  };'
    + '  var q = Object.keys(params).map(function(k){return k+"="+encodeURIComponent(params[k]);}).join("&");'
    + '  window.top.location.href = "' + url + '?page=savecoiff&" + q;'
    + '}'
    + '</script>'
    + '</body></html>';
...
function pageSaveCoiff(e) {
  ...
  try {
    ...
    return HtmlService.createHtmlOutput(
      ...
      + '</head><body style="font-family:sans-serif;background:#0A0A0F;color:#fff;padding:20px;text-align:center">'
      + '<p style="color:#10B981;font-size:18px;font-weight:700">Sauvegarde OK !</p>'
      + '<p><a href="' + adminUrl + '" style="color:#6366F1">Cliquez ici si pas redirige</a></p>'
      + '</body></html>'
    ).setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);
